...
resource "aws_cloudwatch_metric_alarm" "service_cpu_high" {
  count               = var.autoScale_resource_usage ? 1 : 0
  alarm_name          = "${var.ecs_name}_cpu_utilization_high"
  ...
  tags = merge(
    {
      "Name" = format(
        "${var.prefix_name}-${var.ecs_name}-alarm-up"
      )
    },
    var.tags,
  )
...
resource "aws_cloudwatch_metric_alarm" "service_cpu_low" {
  count               = var.autoScale_resource_usage ? 1 : 0
  alarm_name          = "${var.ecs_name}_cpu_utilization_low"
  ...
  tags = merge(
    {
      "Name" = format(
        "${var.prefix_name}-${var.ecs_name}-alarm-down"
      )
    },
    var.tags,
  )
