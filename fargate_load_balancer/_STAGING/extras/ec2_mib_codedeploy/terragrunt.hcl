...
terraform {
    source = local.enabled ? "../../../module_ec2_codedeploy" : null
}

dependency "sg" {
    config_path = "../../baseline/security_groups/sg_mib_ec2_codedeploy"
}

dependency "s3" {
    config_path = "../s3_codedeploy"
}
...
inputs = {
    ...
    prefix_name        = "${local.project}-mib-codedeploy-${local.env}"
    key_pair           = local.key
    volume_size        = "20"
        ...
        Statement = [
            {
                ...
                Resource = "${dependency.s3.outputs.arn}/*"
            },
        ]
    })
