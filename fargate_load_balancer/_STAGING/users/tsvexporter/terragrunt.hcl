...
inputs = {
    name            = "${local.project}-tsvexporter-${local.env}"
    create_policy  = true
    policy_description = "Permite acesso total ao bucket ${local.project} ${local.env} e operações gerais de listagem S3"
    policy_json = jsonencode({
      ...
      Statement = [
        ...
        },
        {
          ...
          Action = "s3:*"
          Resource = [
            "arn:aws:s3:::${local.project}-cms-tsv-export-${local.env}",
            "arn:aws:s3:::${local.project}-cms-tsv-export-${local.env}/*"
          ]
        }
      ]
