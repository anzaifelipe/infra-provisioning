...
inputs = {
    name            = "${local.project}-storagearea-${local.env}"
    create_policy  = true
    policy_description = "Allow full access on bucket ${local.project} storagearea ${local.env} and general S3 listing operations"
    policy_json = jsonencode({
      ...
      Statement = [
        ...
        },
        {
          ...
          Action = "s3:*"
          Resource = [
            "arn:aws:s3:::${local.project}-cms-storagearea-01-${local.env}",
            "arn:aws:s3:::${local.project}-cms-storagearea-01-${local.env}/*"
          ]
        }
      ]
