...
#  description = "ARN of the secret manager to use for repository credentials"
#  type        = string
#}

variable "service_name_second" {
  description = "Set prefix name to resources"
  type        = string
}

variable "image_ecr_url_container_second" {
  description = "value of the ECR where the builder image is stored"
  type        = string
}

variable "image_tag_container_second" {
  description = "value of the ECR where the builder image is stored"
  type        = string
}

variable "container_second_cpu" {
  description = "CPU to use for container, must be equal or less than fargate"
  type        = number
}

variable "container_second_mem" {
  description = "Memory to use for container, must be equal or less than fargate"
  type        = number
}

variable "container_second_environment_variables" {
  description = "Environmental variables in key/pair json encoded map"
  type        = list(any)
  default     = []
}

variable "container2_port_mapping" {
  description = "set ports in key/pair json encoded map"
  type        = list(any)
  default     = []
}